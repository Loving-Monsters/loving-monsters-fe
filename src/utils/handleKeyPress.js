...
export default async function (e, currentUser, currentMap, npcArray = [], setDisableKeys, disableKeys, setCurrentMap, setLoading) {
    e.preventDefault();
    ...
    if (currentUser.current && !disableKeys) {
        ...
        if (currentUser.current.position.x === currentMap.portals.position.x && currentUser.current.position.y === currentMap.portals.position.y) {
            setLoading(true);
            await setCurrentMap(currentMap.portals.nextMap);
